...
import { Send, MessageSquare, Loader2, Users, Megaphone, ShieldAlert, Search } from 'lucide-react';
...
export default function AdminGlobalChatPage() {
  ...
  const [isBroadcast, setIsBroadcast] = useState(true); // Default to broadcast for admins
  ...
  const messagesQuery = useMemoFirebase(() => {
    if (!db || !selectedTeamId || !isAdmin) return null;
    return query(
      collection(db, 'teams', selectedTeamId, 'messages'),
      orderBy('timestamp', 'asc'),
      limit(50)
    );
  }, [db, selectedTeamId, isAdmin]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const messageData = {
      ...
      senderName: `Admin: ${profile?.displayName || 'System'}`,
    ...
    <div className="flex min-h-screen bg-background">
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 pt-16 md:pt-8">
        <header className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold font-headline">League Communications</h1>
            <p className="text-muted-foreground">Broadcast urgent messages or monitor team discussions.</p>
          </div>
          <div className="flex items-center gap-4">
            ...
            {selectedTeamId && (
              <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-full border shadow-sm">
                ...
                <Label htmlFor="broadcast-mode" className="text-sm font-semibold">Broadcast</Label>
                <Switch 
                  id="broadcast-mode" 
                  checked={isBroadcast} 
        ...
        {!selectedTeamId ? (
          <Card className="border-none shadow-md py-20 text-center">
            <CardContent>
              ...
              <h3 className="text-xl font-bold font-headline">Select a Team</h3>
              <p className="text-muted-foreground">Choose a team from the dropdown to start communicating.</p>
            </CardContent>
          </Card>
        ) : (
          <Card className="border-none shadow-xl h-[calc(100vh-200px)] flex flex-col overflow-hidden">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-lg font-headline">
                    {isBroadcast ? 'URGENT: League Broadcast' : activeTeam?.name || 'Loading Chat...'}
                  </CardTitle>
                  <p className="text-xs text-muted-foreground">
                    Direct communication to {activeTeam?.name} families.
                  </p>
                </div>
              </div>
            </CardHeader>
            <CardContent className="flex-1 overflow-y-auto p-6 space-y-4" ref={scrollRef}>
              {loadingMessages ? (
                ...
                <div className="text-center py-20 text-muted-foreground">
                  ...
                  <p>No messages in this team channel yet.</p>
                </div>
              ) : (
                messages.map((msg) => (
                  <div
            ...
            <div className="p-4 border-t bg-white">
              <form onSubmit={handleSendMessage} className="flex gap-2">
                <Input
                  placeholder={isBroadcast ? "Type league broadcast..." : "Message the team..."}
                  value={newMessage}
